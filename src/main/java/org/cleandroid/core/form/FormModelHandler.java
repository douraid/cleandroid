...
import org.cleandroid.core.form.annotation.FormView;
...
public class FormModelHandler implements DependencyProvider{
				...
				for (final Field f2 : varClass.getDeclaredFields()) {
					if (f2.isAnnotationPresent(FormView.class)) {
						...
						if(context instanceof Activity)
							view = ((Activity) context).findViewById(vid);
						else if(context instanceof Fragment)
							view = ((Fragment) context).getView().findViewById(vid);

						if (view instanceof EditText)
							bindEditTextValue(view, returnObject, f2);

						else if (view instanceof Spinner)
							bindSpinnerValue(view, returnObject, f2);

						else if (view instanceof CheckBox || view instanceof RadioButton || view instanceof ToggleButton)
							bindCompoundButtonValue(view, returnObject, f2);
						else if(view instanceof DatePicker)
							bindDatePickerValue(view, returnObject, f2);
						else if(view instanceof TimePicker)
							bindTimePickerValue(view, returnObject, f2);
	...
	public static <T> void handle(Form<T> form, T formModel){
		...
		for(String key:form.getViews().keySet()){
			...
			for(Field f:formModel.getClass().getDeclaredFields()){
				...
				if (view instanceof EditText)
					bindEditTextValue(view, formModel, f);

				else if (view instanceof Spinner)
					bindSpinnerValue(view, formModel, f);

				else if (view instanceof CheckBox || view instanceof RadioButton || view instanceof ToggleButton)
					bindCompoundButtonValue(view, formModel, f);
				else if(view instanceof DatePicker)
					bindDatePickerValue(view, formModel, f);
				else if(view instanceof TimePicker)
					bindTimePickerValue(view, formModel, f);
			}
		}
